...
public class MedicoController {
	...
	public String update(@PathVariable(value="id")Integer id,
			Model model) {
		Medico medico = service.findById(id);
		model.addAttribute("title","Actualizar datos de Medico");
		model.addAttribute("medico",medico);
		return "medico/form";
	}
	...
	public String save(@Valid Medico medico,BindingResult result, Model model,RedirectAttributes message, SessionStatus session) {
		try {
			...
			String msg=medico.getIdpersona()==null?medico.getNombre() +" se ha agregado" : medico.getNombre()+" se ha actualizado";
			service.save(medico);
			session.setComplete();
